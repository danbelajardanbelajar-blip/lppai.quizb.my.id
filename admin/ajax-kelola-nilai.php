...
foreach ($data as $i => $row) {
    ...
    if ($isLulus && $row['reg_id']) {
        if (empty($row['nomor_sertifikat'])) {
            ...
            $cetakSignedBtn = '<a href="'.BASE_URL.'/admin/cetak-sertifikat.php?id='.$row['reg_id'].'&signed=1" target="_blank" class="btn btn-sm btn-primary" style="white-space: nowrap; background-color:#2563eb; border-color:#2563eb; color:white; text-decoration:none; padding:4px 8px; border-radius:4px;" title="No: '.htmlspecialchars($row['nomor_sertifikat']).' (Signed)">✍️ Cetak Signed</a>';
    ...
    $editBtn = '<div style="display: flex; gap: 6px; flex-wrap: nowrap; justify-content: center; align-items: center;">
        ' . $cetakBtn . ' ' . $cetakSignedBtn . ' ' . $lockBtn . '
